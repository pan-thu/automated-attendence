            ...
            : () async {
                ...
                if (didComplete) {
                  messenger.showSnackBar(
                    SnackBar(
                      content: Text(clockIn.statusMessage ?? 'Clock-in recorded.'),
                  ...
                  messenger.showSnackBar(
                    SnackBar(
                      content: Text(clockIn.errorMessage ?? 'Clock-in failed.'),
                      behavior: SnackBarBehavior.floating,
                    ),
                  );
                }
        ...
        Row(
          ...
          children: [
            ...
            Chip(
              ...
              backgroundColor:
                  summary.isActive ? Colors.green.shade100 : Colors.red.shade100,
            ),
          ],
        ),
          ...
          children: [
            ...
            Wrap(
              ...
              children: summary.checks
                  .map(
                    (check) => Chip(
                      label: Text('${check.slot.toUpperCase()}: ${check.status ?? 'pending'}'),
                    ),
                  )
                  .toList(),
            ),
          ...
          children: [
            ...
            ...leaves.map(
              (leave) => ListTile(
                ...
                subtitle: Text('${_formatDate(leave.startDate)} → ${_formatDate(leave.endDate)}'),
              ),
            ),
          ],
        ),
          ...
          children: [
            ...
            ...penalties.map(
              (penalty) => ListTile(
                ...
                trailing: Text('Rs ${penalty.amount.toStringAsFixed(2)}'),
              ),
            ),
          ],
        ),
...
class _CompanyInfoCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final radius = settings.geofenceRadiusMeters;

    final timeWindows = settings.timeWindows.entries
        .map(
          (entry) => '${entry.value.label} (${entry.value.start} – ${entry.value.end})',
        )
        .toList();
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(settings.companyName ?? 'Company Settings',
                style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            Text('Timezone: ${settings.timezone ?? 'Unknown'}'),
            Text('Geofence radius: ${radius != null ? '${radius.toStringAsFixed(0)} m' : 'n/a'}'),
            Text('Geofence enforcement: ${settings.geoFencingEnabled ? 'Enabled' : 'Disabled'}'),

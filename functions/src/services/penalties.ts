...
export const calculateDailyViolations = async (input: CalculateDailyViolationsInput) => {
  ...
  for (const doc of snapshots.docs) {
    ...
    // DEBUG LOGGING
    penaltyLogger.info(`Processing attendance for ${docUserId} on ${date}`, {
      dailyStatus,
    ...
    if (dailyStatus === 'absent' || dailyStatus === 'half_day_absent') {
      ...
      penaltyLogger.info(`Daily status violation detected: ${dailyStatus}`);
    }
    ...
    if (check1Status === 'late') {
      ...
      penaltyLogger.info(`Check1 late violation detected`);
    }
    if (check2Status === 'late') {
      violations.push({ type: 'late', field: 'check2_status' });
      penaltyLogger.info(`Check2 late violation detected`);
    }
    if (check3Status === 'early_leave') {
      violations.push({ type: 'early_leave', field: 'check3_status' });
      penaltyLogger.info(`Check3 early_leave violation detected`);
    }

    penaltyLogger.info(`Total violations found: ${violations.length}`, { violations });

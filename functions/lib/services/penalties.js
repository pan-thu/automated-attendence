...
exports.getPenaltySummary = exports.calculateDailyViolations = exports.calculateMonthlyViolations = exports.acknowledgePenalty = exports.listEmployeePenalties = exports.waivePenalty = void 0;
...
const calculateMonthlyViolations = async (input) => {
    ...
    logger_1.penaltyLogger.info(`Calculating penalties for ${year}-${monthIndex + 1}`, {
    ...
    const attendanceQuery = firestore_2.firestore
        .collection(ATTENDANCE_COLLECTION)
        .where('attendanceDate', '>=', firestore_1.Timestamp.fromDate(start))
        .where('attendanceDate', '<', firestore_1.Timestamp.fromDate(end));
    ...
    for (const [targetUserId, info] of violationSummary.entries()) {
        const violationDocRef = firebase_1.admin.firestore().collection(VIOLATION_HISTORY_COLLECTION).doc();
        const violationRecord = {
            userId: targetUserId,
            violationDate: firestore_1.FieldValue.serverTimestamp(),
        ...
        for (const [status, count] of info.counts.entries()) {
            ...
            await penaltyRef.set({
                ...
                dateIncurred: firestore_1.FieldValue.serverTimestamp(),
                createdAt: firestore_1.FieldValue.serverTimestamp(),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
            });
            penaltiesCreated += 1;
...
exports.calculateMonthlyViolations = calculateMonthlyViolations;
const inferMonthlyViolationCounts = (statuses) => {
